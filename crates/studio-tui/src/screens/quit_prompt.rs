...
pub fn render(frame: &mut Frame, choice: QuitChoice, running: &[ChildSummary]) {
    ...
    let running_names: Vec<String> = running.iter().filter(|c| c.state.is_running()).map(|c| c.info.label.clone()).collect();
    ...
    let lines = vec![
        ...
        Line::raw(""),
        option_line("[K]eep serving — leave models running in the background", choice == QuitChoice::Keep),
        option_line("[S]top everything — stop all models and the daemon now", choice == QuitChoice::Stop),
    ...
    frame.render_widget(
        Paragraph::new(lines).wrap(Wrap { trim: false }).block(Block::bordered().title("Quit CraneStudio?")),
        area,
    );
}

fn option_line(text: &str, selected: bool) -> Line<'static> {
    let style = if selected { Style::new().fg(Color::Black).bg(Color::Cyan).add_modifier(Modifier::BOLD) } else { Style::new() };
