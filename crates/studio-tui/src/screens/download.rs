...
use studio_core::catalog::schema::ModelEntry;
...
impl State {
    ...
    pub fn apply_event(&mut self, event: &Event) {
        match event {
            Event::Started { file, resume_from, total } => {
                let text = if *resume_from > 0 {
                    format!("resuming from {} of {}", crate::fmt::bytes(*resume_from), crate::fmt::bytes(*total))
                } else {
                    format!("starting ({})", crate::fmt::bytes(*total))
                };
                *self.line_mut(file) = text;
            }
            Event::Progress { file, downloaded, total } => {
                #[allow(clippy::cast_precision_loss)]
                let percent = if *total > 0 { (*downloaded as f64 / *total as f64) * 100.0 } else { 0.0 };
                *self.line_mut(file) = format!("{} / {} ({percent:.1}%)", crate::fmt::bytes(*downloaded), crate::fmt::bytes(*total));
            }
...
pub fn start_catalog(app: &mut App, model: &ModelEntry) {
    let Some(variant) = model.variants.iter().min_by_key(|v| v.download_bytes) else {
        app.status_line = Some(format!("{} has no downloadable variants", model.display_name));
        return;
    };
    start(app, format!("{} ({})", model.display_name, variant.id), variant.repo.clone(), variant.revision.clone(), variant.files.clone());
}
...
pub fn start_hf(app: &mut App, repo_id: &str, gguf_file: &str) {
    start(app, repo_id.to_string(), repo_id.to_string(), "main".to_string(), vec![gguf_file.to_string()]);
}

fn start(app: &mut App, label: String, repo: String, revision: String, files: Vec<String>) {
    let cancel = CancellationToken::new();
    app.download = State { label, cancel: Some(cancel.clone()), ..State::default() };
    app.screen = Screen::Download;
    app.status_line = None;

    let dest_dir = studio_core::paths::models_dir().join(&repo).join(&revision);
    let hf_token = studio_core::config::Config::load(&studio_core::paths::config_dir().join("config.ron")).hf_token;
    let tx = app.sender();
    ...
    tokio::spawn(async move {
        ...
        match result {
            Ok(()) => match studio_core::catalog::local::scan(&dest_dir).into_iter().next() {
                Some(candidate) => {
                    let _ = tx.send(BackgroundEvent::DownloadDone(candidate));
                }
                None => {
                    let _ = tx.send(BackgroundEvent::DownloadFailed("downloaded, but couldn't classify the result — check the Local tab".to_string()));
                }
            },
            Err(e) => {
...
pub fn render(app: &mut App, frame: &mut Frame) {
    let [header, body, footer] = Layout::vertical([Constraint::Length(2), Constraint::Min(3), Constraint::Length(1)]).areas(frame.area());

    frame.render_widget(Paragraph::new(format!("Downloading — {}", app.download.label)), header);

    if let Some(err) = &app.download.error {
        frame.render_widget(Paragraph::new(format!("download failed: {err}")).wrap(Wrap { trim: false }).block(Block::bordered().title("Failed")), body);
    } else {
        let text = if app.download.lines.is_empty() {
            "starting…".to_string()
        } else {
            app.download.lines.iter().map(|(file, status)| format!("{file}: {status}")).collect::<Vec<_>>().join("\n")
        };
        frame.render_widget(Paragraph::new(text).wrap(Wrap { trim: false }).block(Block::bordered().title("Progress")), body);
    }

    let footer_text = if app.download.error.is_some() { "[Esc] back" } else { "[Esc] cancel" };
